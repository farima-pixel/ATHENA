...
import utils.GWO as GreyWolf

def run(queue, clouds, tasks):
	
	print("executando TOVEC....")
	...
	for i in queue.get_queue():
		if queue.task_queue_control[i[1]]['status'] == 'PENDING':
			...
			tasks_gwo[i[1]] = [queue.task_queue_control[i[1]]['size'], queue.task_queue_control[i[1]]['value']]
	...
	for cloud in range(len(sorted_clouds)):
		...
		if len(local_tasks) > 0:
			...
			while cloud_capacity > 0:
				...
				temporary_result = GreyWolf.gwo_solution(cloud_capacity, tasks_gwo)
