...
def generate_tasks(task_n, size_n, cycle_n, deadline_n, seed_n):
	...
	task_file = open('TASKS_SIZE_' + str(size_n) + '_CPU_' + str(cycle_n) + '_SEED_' + str(seed_n) + '.txt', 'w')
	...
	for i in range(task_n):
		...
		task_deadline = random.randint(5,deadline_n)
...
for seed in SEEDS:
	for i in range(len(max_size)):
		for j in range(len(max_cycle)):
			generate_tasks(number, max_size[i], max_cycle[j], max_deadline, seed)
